...
// reason to mock since it didn't exist before. A global name-based stub
// (matching Vue Test Utils' own `config.global.stubs` mechanism) keeps every
// *other* test's list/avatar rendering from crashing on those store reads.
// A test that specifically needs to assert on IconFrame's own rendering can
// still override this per-mount via that call's own `global.stubs`.
config.global.stubs = {
    ...config.global.stubs,
    IconFrame: true
};

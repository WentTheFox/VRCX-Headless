...
The same `v*` tag that cuts a Docker release also triggers **`.github/workflows/desktop-release.yaml`**: a 6-leg matrix (Windows/macOS/Linux × x64/arm64) that builds the Electron desktop client, self-signs the Windows and macOS artifacts, and attaches all six to a **draft** GitHub Release for that tag. Windows uses native-arm64 (`windows-11-arm`) and macOS uses two native runners (`macos-13` Intel, `macos-latest` Apple Silicon) rather than cross-compiling or relying on Rosetta, for the same reason in both cases: `node-api-dotnet`'s prebuilt native binary must match the *target* arch, and `pnpm install`'s normal optionalDependency resolution only gets that right when it runs on a matching host.
...
**What's proven vs. genuinely new here — updated after three real `workflow_dispatch` dry runs (2026-08-17), no secrets configured yet:** all 4 non-macOS legs (linux-x64, linux-arm64, win-x64, win-arm64) now pass cleanly and repeatably — this also confirms the `windows-11-arm` runner label is genuinely available to this repo/org, not just a theoretical option. Two real bugs were found and fixed in the process, both logged in git history rather than repeated here in full: `dotnet build` rejects `-r`/`--runtime` and `-a`/`--arch` together (all four RID'd legs failed identically on the first run until `-r` was made to replace `-a` rather than join it), and electron-builder treats `CSC_LINK`/`WIN_CSC_LINK` being merely *present* — even as an empty string — as "sign with this identity," so the original job-level `env:` with an empty-string fallback broke unsigned macOS builds outright; the fix exports those vars only when the matching secret is genuinely non-empty, inside the packaging step itself.

The macOS legs are the one piece still not fully closed out. `macos-arm64`'s build-and-package mechanics (dotnet build, `pnpm run prod-linux`, electron-builder itself) were proven to work on the second dry run — it got all the way to a real, specific failure inside electron-builder's signing step, not a scheduling or build-mechanics problem. But **this account's GitHub-hosted macOS runner capacity is genuinely unreliable**, confirmed despite the repo being public (so this isn't a billing/plan gate): running `macos-x64` and `macos-arm64` as sibling matrix legs left one stuck queued until GitHub auto-cancelled the whole run; splitting them into their own `build-macos` job with `max-parallel: 1` (so they never compete for the same slot) fixed that once, but a third run afterward saw *both* mac legs sit queued indefinitely with neither ever dispatching, requiring a manual cancel. Treat macOS runner availability as intermittent on this account — retry rather than assume a workflow bug if a real release run stalls there, and don't be surprised if it needs more than one attempt. The Windows/Linux legs showed no equivalent flakiness across all three runs.

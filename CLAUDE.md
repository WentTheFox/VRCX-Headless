...
Three surfaces share this one comparison so they can't drift: `npm run server -- check-update` (human-readable, or `--json`; exits `1` specifically when a sync is needed and no fork tag exists for it yet — not an error, the same convention `git diff --exit-code` uses, so the scheduled CI workflow that opens an issue can key off the exit code directly), `GET /api/update-check` (session-authenticated, `checkForUpdateSafe()` — a GitHub API hiccup falls back to the last good cache entry instead of erroring for every connected client), and `src/components/UpdateCheckStatus.vue` (new, fork-owned — a `StatusBar.vue` indicator, gated `isLinux || isWeb` since the concept only means anything for a client actually connected to a headless server, not CefSharp's classic standalone install; hidden entirely when there's nothing to report). The desktop client reaches the HTTP route through a new `vrcx-check-update` IPC handler (`src-electron/main.js`, `window.vrcxDesktopAgent.checkForUpdate()`) for the same CORS reason every other server-facing desktop call already does; the web client just fetches same-origin.

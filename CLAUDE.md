...
The same `v*` tag that cuts a Docker release also triggers **`.github/workflows/desktop-release.yaml`**: Windows and Linux build a full x64+arm64 matrix (`build` job, 4 legs, self-signed on Windows), macOS builds Apple Silicon only and ships **unsigned** (`build-macos` job, 1 leg — see below for both "why Apple Silicon only" and "why unsigned"), and all 5 artifacts attach to a **draft** GitHub Release for that tag. Windows uses a native-arm64 runner (`windows-11-arm`) rather than cross-compiling, for the same reason macOS uses a native Apple Silicon runner: `node-api-dotnet`'s prebuilt native binary must match the *target* arch, and `pnpm install`'s normal optionalDependency resolution only gets that right when it runs on a matching host.

**Draft, not published, deliberately.** This repo has GitHub's "Immutable releases" setting active — once a release is *published*, its assets can never be edited or replaced, only a whole new tag/release can supersede it. Since this is the first-ever run of this pipeline in the fork's history, staying in draft until a human downloads and sanity-checks the real artifacts is the only way to get a look before that point of no return. Switching to immediate-publish later is a documented one-line change (drop `--draft` in the workflow's `publish-release` job) once the pipeline's proven reliable across a few real releases — not built now.

**Signing setup (one-time, per machine that will hold the secret):** run `build-scripts/generate-self-signed-certs.sh` locally (openssl-based, Git-Bash portable) to produce `codesign-windows.pfx` — a self-signed Windows Authenticode cert, `CN=WentTheFox, C=HU, ST=Pest, L=Budapest`, empty passphrase (confirmed live: `signtool.exe` signs real installers fine with `WIN_CSC_KEY_PASSWORD` left unset). Base64-encode it and set one GitHub Actions secret: `WIN_CSC_LINK` — electron-builder's own expected variable name, picked up automatically by its Windows signing step, no extra `win` config needed in `electron-builder.config.js`. Self-signed means exactly that: it reduces some SmartScreen friction (a real signature is present) but does **not** eliminate the "unidentified developer" warning. The `.pfx` file never gets committed — `.gitignore` blocks it by extension — and contains private key material regardless of the empty passphrase, so keep it off anywhere less trusted than the machine that generated it.
...
**What's proven — verified across many real `workflow_dispatch` dry runs (2026-08-17/18):** all 4 Windows/Linux legs (linux-x64, linux-arm64, win-x64, win-arm64) pass cleanly and repeatably, including real Windows signing — `signtool.exe` confirmed actually signing `VRCX Setup <version>.exe`, its uninstaller, and `elevate.exe` with the self-signed cert, timestamp counter-signature included. This also confirms the `windows-11-arm` runner label is genuinely available to this repo/org. Two real bugs were found and fixed along the way (full detail in git history): `dotnet build` rejects `-r`/`--runtime` and `-a`/`--arch` together (all RID'd legs failed identically until `-r` replaced `-a` rather than joining it); and electron-builder treats `CSC_LINK`/`WIN_CSC_LINK` being merely *present* — even as an empty string — as "sign with this identity," so the original job-level `env:` with an empty-string fallback broke unsigned builds outright, fixed by exporting the var only when the matching secret is genuinely non-empty, inside the packaging step itself.

**macOS is Apple Silicon only** (`macos-x64`/`macos-13` never once got a runner dispatched across many attempts — always stuck `queued`, eventually auto-cancelled; `macos-arm64`/`macos-latest` is intermittent but does dispatch and build) **and ships unsigned, both deliberately.** The macOS runner-scheduling unreliability is confirmed real and not a billing/plan gate (the repo is public) — retry rather than assume a workflow bug if a real release run stalls there. The unsigned decision is a separate, harder finding: a self-signed PKCS12 genuinely CAN be imported into a macOS keychain (after working around a real OpenSSL-3.x-vs-Apple-Security-framework PBE-algorithm incompatibility — modern OpenSSL's default AES-256/SHA-256 PKCS12 encryption fails `security import` with "MAC verification failed" regardless of password; explicit `-certpbe PBE-SHA1-3DES -keypbe PBE-SHA1-3DES -macalg SHA1` fixes the import), but electron-builder's own identity-discovery preflight then refuses to use the imported identity anyway, because macOS's Security framework flags any certificate with no trusted CA chain as `CSSMERR_TP_NOT_TRUSTED` — a state every self-signed cert is permanently in, and one electron-builder has no config option to override (it manages its signing keychain internally with no hook to mark an identity trusted before that check runs). A real fix means bypassing electron-builder's built-in mac signing entirely — manual keychain import, `security add-trusted-cert`, and a direct `codesign` call outside its control — explicitly out of scope for now. `build-scripts/generate-self-signed-certs.sh` no longer generates a macOS cert at all; there's nothing for it to sign.

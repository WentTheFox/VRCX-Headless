...
| `src-electron/preload.js` | A `vrcxDesktopAgent` `contextBridge.exposeInMainWorld` (`connectToServer`, `rpc`, `checkTotpSetupNeeded`, `confirmTotpSetup`, and the multi-server `listServers`/`switchServer`/`removeServer`/`setDefaultServer`/`getServerStatus`/`onServerStatusChanged`) beside the existing `interopApi` one | Same reasoning as the `interopApi` bridge — a new IPC surface needs a new bridge entry | beside the existing `contextBridge.exposeInMainWorld('interopApi', ...)` call |

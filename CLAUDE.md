...
**Status (2026-08-17):** All six roadmap phases plus TOTP auth and the Groups-sidebar relay are implemented and verified — server-side by the test suite (`npm run test:server`), client-side by real browser/desktop passes (Chrome via Claude in Chrome; a real Electron build against a real `serve` instance). The one open item: Phase 5's live desktop pass confirmed connection, auth, RPC (db/config/webapi), and app boot — and found and fixed five real bugs in the process (see §8) — but hasn't yet exercised a real Discord presence update or a real game-log line arriving over the agent channel specifically. Everything else in §9 "Known limitations" is a deliberate scope cut, not unfinished work. §8 collapses each phase to what shipped and what merges should watch for — expand it for the roadmap narrative, not for merge mechanics; those live in §2–§6.
...
| 5 | Desktop client as native agent: log forwarding up, overlay/Discord/notification commands down; .NET stops touching SQLite | done, live-desktop verified for connect/auth/RPC/boot — Discord presence and game-log-over-agent specifically not yet exercised live |
...
**Not yet exercised live:** a real Discord presence update or a real game-log line arriving over the agent channel specifically — connection, auth, and the RPC/boot chain are confirmed, but those two capabilities haven't been.
...
- **No fork-adapted Electron CI, and the bundled `.NET`/`node-api-dotnet` footprint hasn't been shrunk.** `.github/workflows/github_actions.yml` is still upstream's own Windows/CefSharp + Azure-signing pipeline; both need a real `dotnet` + Electron toolchain to build and verify against, so neither has been attempted blind.

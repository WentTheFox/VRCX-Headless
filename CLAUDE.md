...
| `src/views/Settings/components/Tabs/SystemTab.vue` | (2026-08-23) New `isRealLinux` computed, `navigator.platform`-based (same pattern as the existing `isMacOS` computed right above it) rather than the `LINUX` compile flag; the `.desktop`-file autostart hint (`t('...startup_linux')`) is now only shown when `isRealLinux`, `undefined` (hidden, `SettingsItem`'s own `v-if="description"`) otherwise. New "Based on VRCX version" row (`upstreamVersion` from the store) right below the existing "Version" row. Removed the "Support" row (`links.discord`) entirely. Also gained `v-if="!noUpdater"` on the General section's "Latest Version" row — found live the same day: that row's `@click="checkForVRCXUpdate"` was unguarded (unlike the separate "VRCX Updater" section below it, already `noUpdater`-gated), so it still queried upstream's own `api0.vrcx.app` and displayed upstream's release name on click even with the self-updater otherwise disabled (`.no-updater`, §9) | `LINUX` means "this is the Electron build" (§1's "Desktop client OS support"), not "running on Linux" — the Electron client also runs on Windows/macOS now, where telling a user to edit a `VRCX.desktop` file is nonsensical; needs the real host OS. The version rows, the Support-link removal, and the `noUpdater` gate are plain UI additions/removals with no smaller seam to alias through | `isRealLinux` beside `isMacOS`; the `v-else` `SettingsItem`'s `:description` binding; the new "Based on VRCX version" `SettingsItem`; the removed "Support" `SettingsItem`; the "Latest Version" `div`'s `v-if="!noUpdater"` |
| `src/components/nav-menu/NavMenu.vue` (+ its test) | (2026-08-23) Reads `currentVersion` from `vrcxUpdaterStore` instead of `appVersion`, and the footer version badge's computed now splits `currentVersion` instead of `appVersion` | `appVersion` now carries this fork's own branding/version (`VRCX Headless <fork-version>`, see `src-electron/main.js`'s entry) — splitting *that* on `'VRCX '` would show `Headless <fork-version>` in the small footer badge. `currentVersion` is the store's own already-brand-stripped computed (`vrcxUpdater.js`'s entry), so reusing it here needs no new parsing logic | the `storeToRefs(VRCXUpdaterStore)` destructure; the `version` computed |

...
| `src/localization/en.json` | (2026-08-23) New `view.settings.general.general.upstream_version: "Based on VRCX version"` key; `view.settings.general.general.support` key removed (its one reference, `SystemTab.vue`'s "Support" row, is gone — see that file's own entry). `view.settings.general.vrcx_updater.updater_disabled` reworded from upstream's "Updater disabled, updates are handled by your package manager." (accurate for e.g. a distro-packaged install, never true here) to pointing at the project's GitHub releases page — this fork has no package-manager install path live yet (the AUR package, §10, is built but dormant until `aur.archlinux.org` registration happens). `nav_menu.github` reworded from "VRCX on GitHub" to "VRCX Headless on GitHub"; `nav_menu.discord` key removed (its one reference, `NavMenuFooter.vue`'s Discord dropdown item, is gone — see that file's own entry) | Only `en.json` was touched — every other locale file's now-unused `support`/`discord` keys are inert (never referenced) and vue-i18n's `fallbackLocale: 'en'` covers the new/reworded keys for every language that hasn't translated them yet, so there's no functional gap, just an untranslated string on non-English locales until someone does | the `general.general` block under `view.settings`; `vrcx_updater.updater_disabled`; `nav_menu.github`/`nav_menu.discord` |

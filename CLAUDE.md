...
**What's proven vs. genuinely new here** — same rigor as everywhere else in this doc: Linux x64/arm64 and the Windows x64 build *mechanics* (dotnet build → `pnpm run prod-linux` → `pnpm run build-electron`) match already-verified patterns (`client-desktop.yaml`'s CI, and a manual live pass documented in `client-desktop/README.md`). Genuinely unverified until a real tag-push CI run proves otherwise: the entire signing pipeline end to end (zero prior signing anywhere in this repo), the `windows-11-arm` runner label's actual availability, and the macOS build in general — no macOS Electron build, signed or not, has ever been exercised in this fork, and `Dotnet/AppApi/Electron/**` has zero macOS branching (folder ops, VRChat autodetect/launch, overlay are all expected-broken on macOS until someone does for it what `GameHandler.cs` already got for Windows).

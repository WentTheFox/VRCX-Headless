...
| `.github/workflows/github_actions.yml` | `npm ci`/`npm run X` → `pnpm install --frozen-lockfile`/`pnpm run X`, `actions/setup-node`'s manual npm-cache-directory dance replaced with `pnpm/action-setup` + `cache: pnpm` (2026-08-17 pnpm migration) | Deliberately touched despite this file being upstream's own untouched Windows/CefSharp + Azure-signing pipeline (§9) — leaving it on `npm ci` after `package-lock.json` was deleted would silently break it outright, worse than the small diff. Every *job* (the actual signing/build steps) stayed untouched | the `Restore dependencies`/`Get npm cache directory`/`npm-cache` steps in both the Linux and macOS build jobs |

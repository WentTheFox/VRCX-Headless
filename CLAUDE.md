...
| `history` | Not `src/**` — `vue-router` itself, inside its own `finalizeNavigation`. Its `history.state` read is already guarded by vue-router's own `isBrowser` check (`typeof document !== 'undefined'`), but `installDocumentPolyfill` above makes that true here for unrelated reasons, so vue-router's own Node-safety check misfires and reaches for a global nothing installs — a `ReferenceError` swallowed into a cosmetic `[VUE_ROUTER_R0011]` console warning with no visible cause (`nostics`, vue-router's diagnostic library, never prints the underlying error). Found by temporarily patching `node_modules/vue-router`'s own bundle to log it | polyfilled in `server/src/globals.js` (`installHistoryPolyfill`) — `mountHeadlessApp()` uses `createMemoryHistory()` exclusively, so only `state` is a real read; the rest of the stub is inert shape-completeness |
...
| `src-electron/main.js` | Phase 5: removed the `SQLite`/`WebApi` .NET object `Init()` calls entirely; added a `vrcx-connect-server` IPC handler (logs into a remote server, opens the `/api/agent` WebSocket, answers forwarded calls with the real `interopApi.callMethod`), a `vrcx-rpc` handler relaying `database`/`config`/`webapi` calls, and a startup gate probing the stored session before loading the real app vs. a new `client-desktop/setup.html`. TOTP work: two more handlers, `vrcx-totp-setup`/`vrcx-totp-confirm`, mirroring `server/src/http-server.js`'s own two routes. Live desktop pass (found real bugs, both fixed): added `vrcx-get-stored-server-url` so `client-desktop/setup.js` can pre-fill the remembered server URL instead of asking again on every `serve` restart; and fixed a genuine pre-existing upstream bug (predates this fork — `74bae434`, "Use Wayland when available") where the `--x11` launch flag only skipped `tryRelaunchWithArgs()`'s own auto-relaunch decision without ever telling Chromium to actually use X11, so on a real Wayland+Vulkan desktop the window silently never became visible (process, tray icon and agent connection all came up fine) while the native Wayland ozone backend failed underneath it — now `app.commandLine.appendSwitch('ozone-platform', 'x11')` when `--x11` is passed | This is `main.js`'s own boot sequence and IPC surface — nothing to alias, the whole point is *changing* what this file does at startup; the ozone fix is a genuine upstream bug, fixed here since there's no smaller seam to alias it through | the `interopApi.getDotNetObject(...)` block near the top; `const x11 = args.includes('--x11')` right after; `ipcMain.handle('vrcx-connect-server'/'vrcx-rpc'/'vrcx-totp-setup'/'vrcx-totp-confirm'/'vrcx-get-stored-server-url', ...)` calls beside the existing `callDotNetMethod` one; `createWindow()`'s `mainWindow.loadFile(...)` call |
| `src-electron/preload.js` | Phase 5: added a `vrcxDesktopAgent` `contextBridge.exposeInMainWorld` (`connectToServer`, `rpc`), beside the existing `interopApi` one. TOTP work: two more bridged calls, `checkTotpSetupNeeded`/`confirmTotpSetup`. Live desktop pass: one more, `getStoredServerUrl`, backing `main.js`'s new IPC handler above | Same reasoning as `interopApi`'s own bridge just above it in this file — a new IPC surface needs a new bridge entry, there's nothing to alias | beside the existing `contextBridge.exposeInMainWorld('interopApi', ...)` call |
...
### Groups sidebar relay — done (2026-08-14)

Found live, then fixed the same session: the Groups sidebar (friends currently in a joinable group instance) was empty on every client — `cachedGroups.size` was a real 130 while `groupStore.groupInstances.length` stayed 0. Root cause traced to the seam table's own documented design, not a bug in it: `groupStore.groupInstances` is populated only by `src/stores/updateLoop.js`'s server-only 5-minute poll (`groupRequest.getUsersGroupInstances()` → the real, unmodified `handleGroupUserInstances()`), and `updateLoop.js` is a deliberate permanent no-op stub on every client (`client-web/shims/update-loop.js`, `client-desktop/shims/update-loop.js`) — "server owns the one real loop, not one per client." So this one piece of server-computed, in-memory-only Pinia state simply never reached a client. Asked the user for a design direction rather than guessing; chosen over "each client redundantly re-polls VRChat itself" and "just document it": **the server relays its own already-computed state**.

<details>
<summary><strong>What shipped</strong> (expand for the design and a real bug the live pass caught)</summary>

- **`server/src/group-instance-relay.js`** (new): wraps `WebApiShim.Execute` (`server/src/shims/webapi.js` — our own seam, not an upstream call site) to catch the one REST endpoint `updateLoop.js` already calls (`users/{id}/instances/groups`, matched by URL substring) and emits its raw, parsed JSON response on a plain `EventEmitter`. Deliberately doesn't touch `updateLoop.js`/`groupCoordinator.js` (both real, unmodified upstream) — same reasoning `pipeline-relay.js` already established: tap at *our* seam, not an upstream call site, per invariants 2/3.
- **`server/src/http-server.js`**: relays that payload over the *same* `/api/stream` connection real pipeline frames use, shaped to look like one (`{type: 'vrcx-headless-group-instances', content: <JSON-string-encoded payload>}`, matching the real wire format's double-encoding) — so `src/services/websocket.js`'s own unmodified `handlePipeline` sees a well-formed frame with an unrecognized `type` and just logs `"Unknown pipeline type"`, exactly its existing behaviour for any type it doesn't know, rather than needing a new switch case (an upstream edit) or a second WebSocket. Also caches the last relayed frame and replays it to each newly-connecting client — found live, not predicted: the underlying poll starts the instant `serve` boots, well before the HTTP server even starts listening, so without a replay every real client would connect *after* that first update and see an empty sidebar for up to 5 minutes regardless.
- **`client-web/bootstrap.js`**: taps the `WebSocket` constructor the same way the server's own `pipeline-relay.js` does — a second, independent `addEventListener('message', ...)` alongside `websocket.js`'s own `.onmessage` (proven side-effect-free by that file's own doc comment: both fire, neither replaces the other) — that recognizes the synthetic frame and calls the real, unmodified `handleGroupUserInstances` directly with the relayed payload. Reuses real business logic (including its own `instanceStore.applyInstance`/`groupStore.cachedGroups` lookups, backed by the client's own already-synced cache) instead of re-deriving the sidebar's shape client-side.

Verified live end-to-end (Claude in Chrome, real account): a fresh login populated the sidebar with 48 real group instances across real groups (`Friday Night Madness`, `Furry`, `Club Sova`, …), each with correct member counts and instance names, with zero console errors beyond the pre-existing, unrelated `FocusWindow` no-op — and exactly one benign `"Unknown pipeline type"` log per relayed frame, confirming `websocket.js`'s own unmodified handling stays undisturbed. 5 new server tests (`server/test/group-instance-relay.test.js`): relays only a successful, URL-matched call; ignores unrelated URLs and failed calls; doesn't throw on a malformed body; wrapping twice doesn't double-emit.

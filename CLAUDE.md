...
**Signing setup (one-time, per machine that will hold the secrets):** run `build-scripts/generate-self-signed-certs.sh` locally (openssl-based, Git-Bash portable) to produce two *separate* self-signed certificates, both `CN=WentTheFox, C=HU, ST=Pest, L=Budapest` — `codesign-windows.pfx` (Authenticode) and `codesign-macos.p12` (Apple codesign). Base64-encode each and set four GitHub Actions secrets: `WIN_CSC_LINK`/`WIN_CSC_KEY_PASSWORD` for Windows, `CSC_LINK`/`CSC_KEY_PASSWORD` for macOS (electron-builder's own expected variable names, picked up automatically by its Windows/mac signing steps — no extra `win`/`mac.identity` config was needed in `electron-builder.config.js`). Self-signed means exactly that: it reduces some SmartScreen/Gatekeeper friction (a real signature is present) but does **not** eliminate the "unidentified developer" warning on either OS, and does **not** satisfy Apple notarization, which requires a real Apple Developer ID certificate. The two `.pfx`/`.p12` files never get committed — `.gitignore` blocks them by extension.

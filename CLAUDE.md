...
**Signing setup (one-time, per machine that will hold the secrets):** run `build-scripts/generate-self-signed-certs.sh` locally (openssl-based, Git-Bash portable) to produce two *separate* self-signed certificates, both `CN=WentTheFox, C=HU, ST=Pest, L=Budapest` — `codesign-windows.pfx` (Authenticode) and `codesign-macos.p12` (Apple codesign), both with an **empty passphrase** deliberately (electron-builder treats a missing `CSC_KEY_PASSWORD`/`WIN_CSC_KEY_PASSWORD` as empty, and since these certs are already self-signed with no CA trust to leverage regardless, a passphrase would only protect the raw file if it leaked — accepted tradeoff for needing only 2 secrets instead of 4). Base64-encode each and set two GitHub Actions secrets: `WIN_CSC_LINK` for Windows, `CSC_LINK` for macOS (electron-builder's own expected variable names, picked up automatically by its Windows/mac signing steps — no extra `win`/`mac.identity` config was needed in `electron-builder.config.js`). Self-signed means exactly that: it reduces some SmartScreen/Gatekeeper friction (a real signature is present) but does **not** eliminate the "unidentified developer" warning on either OS, and does **not** satisfy Apple notarization, which requires a real Apple Developer ID certificate. The two `.pfx`/`.p12` files never get committed — `.gitignore` blocks them by extension, but since they're passphrase-free, keep the files themselves off anywhere less trusted than this machine.

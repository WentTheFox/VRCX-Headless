...
**Packaging**: `client-desktop/README.md` documents the real two-stage build (`dotnet build` → `pnpm run prod-linux && pnpm run build-electron`), traced from `.github/workflows/github_actions.yml`'s CI job rather than invented, since that workflow itself has no fork-adapted equivalent (§9).
...
**Found while wiring `isWeb` into `StatusBar.vue`: `WEB` was never added to `eslint.config.mjs`'s globals list or `vitest.config.js`'s `define` block, even though `src/vite.config.js` (§5) has defined it for the real build since it was introduced.** `pnpm run lint` was already failing on two unrelated pre-existing files that reference `WEB` (`src/stores/vrcx.js`, `src/plugins/interopApi.js`) before this change — not something this session's other edits caused, just never noticed. Fixed by adding `WEB: 'readonly'` to `eslint.config.mjs` and `WEB: JSON.stringify(false)` to `vitest.config.js`'s `define` block (matching the existing `WINDOWS: true, LINUX: false` desktop-shaped test default) — both logged in §5.

</details>

<details>
<summary><strong>2026-08-17: upstream sync to master HEAD, and an update checker</strong></summary>

**Synced to upstream `master` HEAD (112 commits since the last tagged sync, `v2026.07.18`) as a deliberate stress-test of §6's sync procedure against real, uncurated drift.** 5 real conflicts, matching §6.1's predictions plus one it didn't call out (`src-electron/main.js` — our CA-cert self-relaunch gate and `dotnetSetup()` both landing at the same spot upstream refactored). Two silent, non-conflict-flagged casualties found only by manual lint/diff auditing, not by the merge process itself: a dropped `spawn` import and a dropped `tryRelaunchWithArgs()` call, both because upstream added its own new line at the exact same textual position — worth remembering that git's 3-way merge can silently drop a line with zero warning when that happens. `pnpm test` came back with 144/2111 failures; all individually verified (absence from this table, plus a zero-diff check against every implicated file on `upstream/master`) to be upstream's own pre-existing breakage at that commit, not anything this fork or this merge caused — `pnpm run test:server` (the split machinery this fork actually owns) passed clean. Separately, 417 files needed reformatting to match this fork's `.oxfmtrc.json` (upstream's own style drift) — caught by real CI, not local checks, since Windows `core.autocrlf` was masking real signal in every local formatter run all session.

Upstream's own "Runtime platform args" commit (`5afeb01a`) moved `LINUX`/`WINDOWS` from `src/vite.config.js`'s compile-time `define` block to runtime injection (`contextBridge.exposeInMainWorld` in `src-electron/preload.js`, a new CEF message handler for CefSharp) so they can ship one shared `build/html` for both their CefSharp and Electron clients instead of building twice. This fork kept the compile-time `define` approach instead of adopting that — deliberately, not an oversight: our aliasing (`clientDesktopAliases`/`clientWebAliases`/no aliasing at all for CefSharp) already forces separate builds per platform regardless, so the problem upstream's change solves doesn't apply here, and `define` remains fully compatible with upstream's `src/**` code either way (same bare `LINUX`/`WINDOWS` identifiers, just resolved differently). Upstream's own runtime-injection lines merged in harmlessly dead (nothing in our bundle reads `window.LINUX` once `define` has already replaced the bare identifier at build time) — left in place rather than removed, since editing upstream-owned code to delete otherwise-harmless upstream-owned code isn't worth it.

**Update checker: does this fork's last sync lag behind upstream's latest VRCX release, and if so, is there already a fork build for it?** `server/src/update-check.js` compares `readVersion()` (this fork's currently-synced `Version` file) against `vrcx-team/VRCX`'s `/releases/latest` on GitHub, and — only when those differ — checks whether `WentTheFox/VRCX-Headless` has a **tag** (not a GitHub Release; found live, this fork has never actually published a Release object, only pushed tags per §10's "cutting a release," so the `/releases` endpoint always returns empty and would report a false gap on every single check) whose name starts with `v<latestVrcxVersion, dots stripped>.`. Deliberately uses `node:https` directly rather than `fetch` — found live: a CLI invocation's `process.exit()` right after the request resolves crashes Node with a libuv assertion on Windows specifically, because `fetch`'s (undici's) pooled keep-alive socket is still mid-close at that moment; `info` and every other CLI command are unaffected since none of them make an outbound HTTP call at all. Results cache in-memory for 6h (`--force` bypasses it) — upstream releases every few weeks, no need to re-hit GitHub's unauthenticated rate limit (60/hr/IP) on every poll.

Four surfaces share this one comparison so they can't drift: `pnpm run server -- check-update` (human-readable, or `--json`; exits `1` specifically when a sync is needed and no fork tag exists for it yet — not an error, the same convention `git diff --exit-code` uses), `GET /api/update-check` (session-authenticated, `checkForUpdateSafe()` — a GitHub API hiccup falls back to the last good cache entry instead of erroring for every connected client), `src/components/UpdateCheckStatus.vue` (new, fork-owned — a `StatusBar.vue` indicator, gated `isLinux || isWeb` since the concept only means anything for a client actually connected to a headless server, not CefSharp's classic standalone install; hidden entirely when there's nothing to report), and `.github/workflows/upstream-sync-check.yaml` (new, fork-only — a daily cron plus `workflow_dispatch`, running `check-update --json --force` and opening an issue off `result.issueTitle`/`issueBody` when the exit code says one's needed, labelled `upstream-sync-needed` and deduplicated by searching for an already-open issue with that label first so a sync that's still pending after several days doesn't accumulate a new issue every run). The desktop client reaches the HTTP route through a new `vrcx-check-update` IPC handler (`src-electron/main.js`, `window.vrcxDesktopAgent.checkForUpdate()`) for the same CORS reason every other server-facing desktop call already does; the web client just fetches same-origin.

**npm → pnpm, with a `minimumReleaseAge` supply-chain gate.** `package-lock.json` is gone; `pnpm-lock.yaml` + `pnpm-workspace.yaml` (new, fork-owned) replace it. `pnpm-workspace.yaml`'s `minimumReleaseAge: 20160` (14 days, in minutes) refuses to resolve any dependency version published more recently than that window — a compromised-maintainer-account publish is typically caught and pulled within hours to a few days, so this trades "always get the literal newest patch" for "don't be the one who finds out a package got hijacked." It's a resolution-time gate, not a lockfile-verification one *by default* — `pnpm import` (the standard npm→pnpm migration path) preserves whatever was already resolved, including versions that predate this policy, and a plain `pnpm install` afterward errors loudly (`ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION`) rather than silently accepting them; the fix is `pnpm clean --lockfile && pnpm install` to force a compliant re-resolution, which is what actually happened here — `node-api-dotnet` had to drop back from the `^0.9.23` this fork's own 2026-08-17 upstream sync had just bumped to (published within the window, and the *only* version satisfying that range, so there was no compliant fallback) to `^0.9.21` (what this fork ran before that sync anyway, and already 14+ days old).

Found live during the migration, not predicted: pnpm's strict `node_modules` linking (no flat-hoisting fallback the way npm's does) caught a genuine phantom dependency — `server/src/app.js` imports `@vue/runtime-core` directly, but nothing declared it; it only ever resolved under npm by accident of `vue`'s own transitive copy getting hoisted to the top level. Fixed by declaring it explicitly in both `package.json` and `server/package.json` (`check-deps.js`-enforced, same as every other shared dependency) rather than working around pnpm's stricter behaviour — this is exactly the class of bug that behaviour exists to catch.

`pnpm-workspace.yaml` also carries `allowBuilds` (pnpm blocks a dependency's install/postinstall scripts by default unless approved here) — migrated straight from `package.json`'s old, unenforced `allowScripts` field (same five packages, all previously `true`; that field was never actually wired to a real npm tool, just inert config). `oxlint`/`eslint-plugin-oxlint` got pinned to an exact version (no `^`) rather than left floating — a fresh pnpm resolution drifted to a newer minor that flagged pre-existing, upstream-owned code `npm`'s locked version never had, and while `oxlint`'s own CI step runs with `continue-on-error: true` (so this was never actually going to break anything), pinning removes the surprise rather than relying on that safety net. Every `.github/workflows/*.yaml`/`.yml` and `server/Dockerfile` updated to match (`pnpm/action-setup` + `actions/setup-node`'s `cache: pnpm` in place of the old npm-cache dance, `pnpm install --frozen-lockfile` for `npm ci`); the Dockerfile's two npm stages both needed `corepack enable && corepack prepare pnpm@11.22.0 --activate` first (Alpine's `node` image doesn't ship pnpm), and the `deps` stage — which installs only `server/package.json`, standalone, with no lockfile of its own — needed `pnpm-workspace.yaml` copied in too, or `minimumReleaseAge` wouldn't apply to that fresh resolution at all.

**All 17 distinct `uses:` actions, across every workflow (fork-owned and the upstream-owned `github_actions.yml` alike), pinned to an exact commit SHA with a trailing `# vX` comment** — `actions/checkout@d23441a...  # v6`, not `actions/checkout@v6`: a tag is a mutable pointer an upstream repo (or an attacker who compromises one) can silently repoint at different code; a commit SHA can't be. Resolved via `gh api repos/<owner>/<repo>/commits/<tag>` per action, applied with `sed` (a `#`-delimited first attempt collided with the `# vX` comment text itself and had to be redone with `|` as the delimiter). `.github/workflows/client-desktop.yaml`'s trigger `paths:` still listed the now-deleted `package-lock.json` from before the pnpm migration above — caught and fixed to `pnpm-lock.yaml` in the same pass, since an unpinned trigger path is the kind of thing that silently stops firing rather than erroring.
...
#### Desktop client release artifacts (2026-08-18)

The same `v*` tag that cuts a Docker release also triggers **`.github/workflows/desktop-release.yaml`**: Windows and Linux build a full x64+arm64 matrix (`build` job, 4 legs), macOS builds Apple Silicon only (`build-macos` job, 1 leg — see below for why Intel was dropped), and all 5 artifacts get self-signed (Windows/macOS) and attached to a **draft** GitHub Release for that tag. Windows uses a native-arm64 runner (`windows-11-arm`) rather than cross-compiling, for the same reason macOS uses a native Apple Silicon runner: `node-api-dotnet`'s prebuilt native binary must match the *target* arch, and `pnpm install`'s normal optionalDependency resolution only gets that right when it runs on a matching host.

**Draft, not published, deliberately.** This repo has GitHub's "Immutable releases" setting active — once a release is *published*, its assets can never be edited or replaced, only a whole new tag/release can supersede it. Since this is the first-ever run of self-signed cross-platform signing in this fork's history, staying in draft until a human downloads and sanity-checks the real artifacts is the only way to get a look before that point of no return. Switching to immediate-publish later is a documented one-line change (drop `--draft` in the workflow's `publish-release` job) once the pipeline's proven reliable across a few real releases — not built now.

**Signing setup (one-time, per machine that will hold the secrets):** run `build-scripts/generate-self-signed-certs.sh` locally (openssl-based, Git-Bash portable) to produce two *separate* self-signed certificates, both `CN=WentTheFox, C=HU, ST=Pest, L=Budapest` — `codesign-windows.pfx` (Authenticode, empty passphrase) and `codesign-macos.p12` (Apple codesign, auto-generated real passphrase). **The two are deliberately asymmetric**, found live (2026-08-18) after ruling out every other explanation: Windows genuinely works with an empty password (`signtool.exe` confirmed signing real installers with `WIN_CSC_KEY_PASSWORD` left unset), but macOS's Keychain (`security import`, what electron-builder shells out to for codesign) rejects a truly empty PKCS12 password outright with "MAC verification failed" — reproduced identically across a modern-format cert, a `-legacy`-format cert, and both unset and explicit-empty-string `CSC_KEY_PASSWORD` forms, and confirmed via electron-builder's own source (`platformPackager.js`) that it really does pass a literal empty string in that case, not some other masked value. Base64-encode each `.pfx`/`.p12` and set three GitHub Actions secrets: `WIN_CSC_LINK` for Windows, `CSC_LINK` + `CSC_KEY_PASSWORD` (the real macOS password, pasted as-is, not base64'd) for macOS — electron-builder's own expected variable names, picked up automatically by its Windows/mac signing steps, no extra `win`/`mac.identity` config needed in `electron-builder.config.js`. Self-signed means exactly that: it reduces some SmartScreen/Gatekeeper friction (a real signature is present) but does **not** eliminate the "unidentified developer" warning on either OS, and does **not** satisfy Apple notarization, which requires a real Apple Developer ID certificate. The two `.pfx`/`.p12` files never get committed — `.gitignore` blocks them by extension — and both contain private key material regardless of passphrase, so keep them off anywhere less trusted than the machine that generated them.
...
**What's proven vs. genuinely new here — updated after five real `workflow_dispatch` dry runs (2026-08-17):** all 4 Windows/Linux legs (linux-x64, linux-arm64, win-x64, win-arm64) pass cleanly and repeatably, including with real signing secrets configured — `signtool.exe` was confirmed actually signing `VRCX Setup <version>.exe`, its uninstaller, and `elevate.exe` with the self-signed cert, timestamp counter-signature included. This also confirms the `windows-11-arm` runner label is genuinely available to this repo/org, not just a theoretical option. Three real bugs were found and fixed along the way, logged in git history rather than repeated here in full: `dotnet build` rejects `-r`/`--runtime` and `-a`/`--arch` together (all RID'd legs failed identically until `-r` replaced `-a` rather than joining it); electron-builder treats `CSC_LINK`/`WIN_CSC_LINK` being merely *present* — even as an empty string — as "sign with this identity," so the original job-level `env:` with an empty-string fallback broke unsigned builds outright, fixed by exporting those vars only when the matching secret is genuinely non-empty, inside the packaging step itself; and the cert-generation script originally prompted for a PKCS#12 passphrase that added no real protection (self-signed certs have no CA trust to leverage regardless) while doubling the required secrets, so it now generates both certs with an empty passphrase — `WIN_CSC_KEY_PASSWORD`/`CSC_KEY_PASSWORD` don't exist, only `WIN_CSC_LINK`/`CSC_LINK`.

**macOS is Apple Silicon only, and the Intel leg was dropped entirely rather than left broken.** Across five dry runs, `macos-x64`/`macos-13` never once got a runner dispatched — always stuck `queued` until manually cancelled. `macos-arm64`/`macos-latest` worked twice early on (proving the build-and-package mechanics — dotnet build, `pnpm run prod-linux`, electron-builder, `CSC_LINK` signing — genuinely function, including a real failure at the time inside electron-builder's own signing step, not a scheduling problem), but then also started stalling on later attempts. This account's GitHub-hosted macOS runner capacity is confirmed unreliable — not a billing/plan gate, since the repo is public — and got worse across repeated attempts in a short window, consistent with some form of soft throttling. Given Intel had a 0-for-5 record versus Apple Silicon's intermittent-but-sometimes-working one, `build-macos` now only builds `macos-arm64`; if it stalls on a real release run, retry later rather than assume a workflow bug — the Windows/Linux legs showed no equivalent flakiness across any run.

`Dotnet/AppApi/Electron/**` still has zero macOS branching regardless of build mechanics — folder ops, VRChat autodetect/launch, and overlay remain expected-broken on macOS until someone does for it what `GameHandler.cs` already got for Windows.

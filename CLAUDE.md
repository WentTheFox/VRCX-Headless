...
| 2b | Pinia-in-Node: the background stores and the `updateLoop` daemon | **in progress** — steps 1,2,3,4,5,6,7,9/9 done; 8 left |
...
Ordered so each step is independently verifiable. Steps 1 through 7 and 9 are done (2026-08-14); only 8 is left. 9 shipped out of order — eager instantiation in step 5 reached `AppApi` immediately rather than on first real use, so it stopped being optional.

<details>
<summary><strong>Steps 1–7, 9 — done</strong> (expand for what actually shipped vs. what was originally planned)</summary>

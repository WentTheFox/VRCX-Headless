...
export async function initInteropApi(isVrOverlay = false) {
    if (isVrOverlay) {
        ...
        if (WINDOWS) {
            ...
            // under PLATFORM=web). LogWatcher/Discord/AssetBundleManager
            // are desktop-only capabilities with no web equivalent yet —
            // left unset on purpose rather than stubbed speculatively.
